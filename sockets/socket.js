...
io.on("connection", (client) => {
    ...
    client.on("enable-sentence", async(payload) => {
        io.emit('sentenceEnabled', payload)
    });
    client.on("player-sentence", async(payload) => {
        io.emit('sentencedPlayer', payload)
    });
    client.on("show-next", async(payload) => {
        io.emit('nextShowed', payload)
    });
    client.on("send-result", async(payload) => {
        io.emit('resultSended', payload)
    });
    client.on("finish-game", async(payload) => {
        io.emit('gameFinished', payload)
    });

    client.on("open-hand", async(payload) => {
        io.emit('handOpened', payload)
    });

    client.on("close-hand", async(payload) => {
